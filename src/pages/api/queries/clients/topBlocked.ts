 ...
 */
export type IGetTopClientsResponseData = ITopBlockedClientsData;

/**
 * GET endpoint for `/api/queries/clients/topBlocked`
 *
 * @remarks
 ...
 */
const handleGetTopBlockedClients = (
  req: NextApiRequest,
 ...
 *
 * @remarks
 * HTTP API endpoint `/api/queries/clients/topBlocked`
 *
 * @remarks
...
const requestHandler = (req: NextApiRequest, res: NextApiResponse) => {
  ...
  switch (method) {
    case 'GET': {
      handleGetTopBlockedClients(req, res);
      break;
    }
    default: {
      logger.error({
